odoo.define('funnel.button.dynamic.snippet', function (require) {
    ...
    publicWidget.registry.funnel_buttons_dynamic_snippet = publicWidget.Widget.extend({
        ...
        start: function () {
            ...
            this.$el.append("<input type=\"hidden\" class=\"product_id\" name=\"product_id\" value='" + productId +"'/>");
            this.$el.append("<input type=\"hidden\" class=\"add_qty\" name=\"add_qty\" value='" + productQty +"'/>");

            if (successRedirect) {
                this.$el.append("<input type=\"hidden\" class=\"success_redirect\" name=\"success_redirect\" value='" + successRedirect +"'/>");
            }
            if (failureRedirect) {
                this.$el.append("<input type=\"hidden\" class=\"failure_redirect\" name=\"failure_redirect\" value='" + failureRedirect +"'/>");
            }

            this.$el.append("<input type=\"hidden\" class=\"csrf_token\" name=\"csrf_token\" value='" + odoo.csrf_token +"'/>");
